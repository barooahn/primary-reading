...
export function calculateReadingMetrics(userAnswers: Array<{ score: number; [key: string]: string | number | boolean }>, readingSessions: Array<{ reading_time: number; [key: string]: string | number | boolean }>): Partial<UserPerformanceData> {
  ...
  const accuracyByType = userAnswers.reduce((acc, answer) => {
    ...
    if (answer.question && typeof answer.question === 'object' && answer.question !== null) {
      const question = answer.question as Record<string, unknown>;
      if ('question_type' in question && typeof question.question_type === 'string') {
        questionType = question.question_type;
      }
    }

    if (!acc[questionType]) acc[questionType] = { correct: 0, total: 0 };

    acc[questionType].total++;
    if (answer.is_correct) acc[questionType].correct++;
  ...
  const totalWordsRead = readingSessions.reduce((sum, session) => {
    let wordCount = 0;
    if (session.story && typeof session.story === 'object' && session.story !== null) {
      const story = session.story as Record<string, unknown>;
      if ('word_count' in story && typeof story.word_count === 'number') {
  ...
  const totalReadingTime = readingSessions.reduce((sum, session) => {
    const readingTime = typeof session.reading_time_minutes === 'number' ? session.reading_time_minutes : 0;
