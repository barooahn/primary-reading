...
export function SimpleLoginForm({ onModeChange }: SimpleLoginFormProps) {
  ...
  return (
    <ErrorBoundary
      fallback={
        <Card className="w-full max-w-md mx-auto bg-white/95 backdrop-blur-xl border-2 border-red-200/60 shadow-[0_8px_30px_rgba(0,0,0,0.1)]">
          <CardHeader className="text-center">
            <CardTitle className="text-xl font-bold text-red-800">Login Unavailable</CardTitle>
    ...
    >
      ...
      <CardContent className="space-y-5">
        <form onSubmit={handleAuth} className="space-y-5" noValidate role="form" aria-label={mode === 'signin' ? 'Sign in to your account' : 'Create a new account'} aria-describedby="form-description">
          <div id="form-description" className="sr-only">
            {mode === 'signin'
              ? 'Enter your email and password to sign in to your PrimaryReading account'
              : 'Create a new PrimaryReading account by entering your email and creating a password'
            }
          </div>
          {/* Email Field */}
