-- Fixed migration script to resolve schema mismatch
...
BEGIN
  ...
  BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'story_type') THEN
      ...
      RAISE NOTICE 'Created story_type enum';
    END IF;
  END $enum$;
...
-- Handle reading_level enum conversion more carefully
DO $$
BEGIN
  -- First, create the reading_level enum type if it doesn't exist
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'reading_level_enum') THEN
    CREATE TYPE reading_level_enum AS ENUM ('beginner', 'intermediate', 'advanced');
    RAISE NOTICE 'Created reading_level_enum type';
  END IF;

  -- Check if reading_level column exists and is TEXT type
  IF EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'stories' 
    ...
    -- First, make sure all values are valid enum values
    UPDATE stories 
    SET reading_level = 'beginner' 
    WHERE reading_level NOT IN ('beginner', 'intermediate', 'advanced');
    
    -- Convert the column type using a temporary column approach
    ALTER TABLE stories ADD COLUMN reading_level_temp reading_level_enum;
    UPDATE stories SET reading_level_temp = reading_level::reading_level_enum;
    ALTER TABLE stories DROP COLUMN reading_level;
    ALTER TABLE stories RENAME COLUMN reading_level_temp TO reading_level;
    
    RAISE NOTICE 'Converted reading_level column to enum type';
  ELSIF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'stories' 
    AND column_name = 'reading_level'
    ...
    -- Add reading_level column if it doesn't exist
    ALTER TABLE stories ADD COLUMN reading_level reading_level_enum NOT NULL DEFAULT 'beginner';
    RAISE NOTICE 'Added reading_level column with enum type';
  END IF;
END $$;

-- Create the story_segments table if it doesn't exist
